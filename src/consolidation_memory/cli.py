...
def cmd_init(quick: bool = False):
    ...
    from consolidation_memory.config import get_config_path
    ...
    if existing:
        if quick:
            ...
            print("Quick setup keeps the existing config.")
            return
        print(f"Config already exists at: {existing}")
    ...
    _write_default_config(embed_backend=embed_backend, llm_backend=llm_backend)
...
def cmd_warmup(*, claims: bool = False, as_json: bool = False) -> None:
    ...
    try:
        if claims:
            with override_config(WARMUP_PRIME_CLAIM_CACHE=True):
                ...
                stats = warm_recall_caches(client)
        else:
            from consolidation_memory.tool_adapter import warm_recall_caches
    ...
    ready = "yes" if stats.get("knowledge_cache_ready") else "no"
    print(
        "Warmup complete "
...
def cmd_ui(host: str = "127.0.0.1", port: int = 8080, open_browser: bool = True) -> None:
    ...
    print(f"Starting browser UI on {url}")
    if open_browser:
        webbrowser.open(url)
    app = create_app()
    uvicorn.run(app, host=host, port=port)
...
def main():
    ...
    p_init = sub.add_parser("init", help="Interactive first-run setup")
    p_init.add_argument(
        "--quick",
        action="store_true",
    ...
    p_warmup = sub.add_parser(
        "warmup",
        help="Pre-build recall embedding caches before MCP/IDE sessions",
    )
    p_warmup.add_argument(
        "--claims",
        action="store_true",
        help="Also warm active claim embeddings (off by default)",
    )
    p_warmup.add_argument(
        "--json",
        action="store_true",
    ...
    if args.command is None or args.command == "serve":
        ...
        cmd_warmup(claims=args.claims, as_json=args.json)
