...
def _hold_lock_then_release(lock_path: str, hold_seconds: float, order: mp.Queue) -> None:
...
def _acquire_after_peer(lock_path: str, order: mp.Queue) -> None:
    lease = ProcessWriteLease(Path(lock_path), timeout_seconds=10.0)
    with lease.acquire():
        order.put("acquired")
...
class TestProcessWriteLease:
    def test_serializes_cross_process_access(self, tmp_path):
        ...
        holder = mp.Process(
            ...
            args=(str(lock_path), 0.35, order),
        )
        waiter = mp.Process(
            target=_acquire_after_peer,
            args=(str(lock_path), order),
        )
